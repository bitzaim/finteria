<template>
  <NuxtLayout class="cryptocurrency" name="top-background">
    <section class="intro">
      <div class="container">
        <h1 class="title title_without-border">{{ $t("cryptocurrency.title") }}</h1>

        <p class="intro__lead">{{ $t("cryptocurrency.lead") }}</p>
        <p>{{ $t("cryptocurrency.description1") }}</p>
        <p>{{ $t("cryptocurrency.description2") }}</p>
      </div>
    </section>

    <section class="tools">
      <div class="container tools__container">
        <h2 class="visually-hidden">{{ $t("cryptocurrency.toolsSectionTitle") }}</h2>

        <ToolCarousel :items="toolsData" />
      </div>
    </section>

    <section class="trading">
      <div class="container">
        <h2 class="title title_border-after">{{ $t("cryptocurrency.tradingSectionTitle") }}</h2>

        <p class="trading__text">{{ $t("cryptocurrency.tradingSectionText") }}</p>
        ...
        <div class="trading__info">
          <picture>
            ...
            <img
              ...
              :alt="$t('cryptocurrency.tradingSectionTitle')"
              width="340"
              height="300"
            />
          </picture>
    ...
    <section class="why-finteria">
      <div class="container why-finteria__container">
        <div class="why-finteria__info">
          <h2 class="title title_border-after">{{ $t("cryptocurrency.whyFinteriaSectionTitle") }}</h2>
          ...
          <NuxtLink class="button button_yellow cryptocurrency__get-started why-finteria__get-started" to="/">{{
            $t("cryptocurrency.getStarted")
          }}</NuxtLink>
        </div>

        <ToolsTable class="why-finteria__tools-table" :items="toolsData" />
      </div>
    </section>
...
useHead({
  title: t("cryptocurrency.title"),
  meta: [
    {
      name: "description",
      content: t("cryptocurrency.lead"),
    },
  ],
});
